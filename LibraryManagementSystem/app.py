...
def cancel_operation():
    ...
    if reader:
        if reader['status'] in ('正常', '停借'):
            report_loss_sql = "UPDATE Readers SET status='注销' WHERE card_number=%s"
            cursor.execute(report_loss_sql, (card_number,))
            conn.commit()
            cursor.close()
            conn.close()
...
def borrow_records():
    ...
    if condition:
        where = f" WHERE isbn=(SELECT b.isbn FROM Books b WHERE {condition} LIKE %s)"
        count_sql = ''.join(("SELECT COUNT(*) AS total FROM Borrow_records",where))
        data_sql = base_sql + where + " LIMIT %s OFFSET %s"
        count_params = [value,]
        data_params = [value, per_page, (page - 1) * per_page]
